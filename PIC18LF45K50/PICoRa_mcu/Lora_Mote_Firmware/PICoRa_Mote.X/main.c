...
extern void PICoRa_test_powerup(void);
...
void main(void)
{
    ...
    USBDeviceAttach(); //indicates to the USB host that the USB device has been attached to the bus
    // MCLR RN Module
    MOD_RESET_LAT = ENABLE; //USB stack hardware
    moteApp_delayms(50);
    MOD_RESET_LAT = DISABLE; //USB stack hardware
    moteApp_delayms(500);
    // Interrupts Enabled
    PEIE = 1;
    GIE = 1;
    // Test Hardware
    moduleResync(); //checking module
    moteApp_delayms(20);
    moteApp_clearCommand(); //Prepare Buffer for Response
    moteApp_delayms(20);
    
    powerOnStatus = DATAEE_ReadByte(0x00); //Read Data EEPROM Module APIs
    if ( powerOnStatus != 0x55)
    {
        PICoRa_test_powerup(); //test overall function - button, LED, sensor, oLED monitor
        DATAEE_WriteByte(0x00, 0x55); //Write Data EEPROM Module APIs
    }
    ...
    while(1)
    {
        ...
        switch (operationType)
        {
            ...
            break;
            case MOTE:
                if (moteHandler() == MOTE_SWAP) //mote function
                {
